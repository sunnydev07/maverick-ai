...
import { Keyboard, Mic, Volume2, Settings, Save, AlertCircle, CheckCircle } from 'lucide-react'
...
export function SettingsPanel() {
  ...
  const [testing, setTesting] = useState<'worker' | 'hotkey' | null>(null)

  // Load settings on mount via IPC
  useEffect(() => {
    const loadSettings = async () => {
      try {
        const loaded = await window.electronAPI['load-settings']()
        if (loaded) {
          setSettings(loaded as SettingsState)
        }
      } catch (error) {
        ...
        if (stored) {
          try {
            ...
            console.error('[v0] Failed to parse localStorage:', e)
          }
        }
      } finally {
        setLoading(false)
  ...
  const handleSave = async () => {
    try {
      const response = await window.electronAPI['save-settings'](settings)
      if (response.success) {
      ...
      const errorText = error instanceof Error ? error.message : 'Failed to save settings'
  ...
  const handleTestWorkerUrl = async () => {
    if (!settings.workerUrl.trim()) {
      setMessage({ type: 'error', text: 'Worker URL cannot be empty' })
      return
    }

    setTesting('worker')
    try {
      const response = await window.electronAPI['test-worker-url']({ url: settings.workerUrl })
      setMessage({ type: response.success ? 'success' : 'error', text: response.message })
    } catch (error) {
      const errorText = error instanceof Error ? error.message : 'Failed to test worker URL'
      setMessage({ type: 'error', text: errorText })
      console.error('[v0] Test worker error:', error)
    } finally {
      setTesting(null)
      setTimeout(() => setMessage({ type: null, text: '' }), 3000)
    }
  ...
  const handleTestHotkey = async () => {
    if (!settings.hotkeyCombo?.trim()) {
      setMessage({ type: 'error', text: 'Hotkey cannot be empty' })
      return
    }

    setTesting('hotkey')
    try {
      const response = await window.electronAPI['test-hotkey']({ combo: settings.hotkeyCombo })
      if (response.conflict) {
        setMessage({ type: 'error', text: `${response.message} (Conflict detected)` })
      } else {
        setMessage({ type: response.success ? 'success' : 'error', text: response.message })
      }
    } catch (error) {
      const errorText = error instanceof Error ? error.message : 'Failed to test hotkey'
      setMessage({ type: 'error', text: errorText })
      console.error('[v0] Test hotkey error:', error)
    } finally {
      setTesting(null)
      setTimeout(() => setMessage({ type: null, text: '' }), 3000)
    }
  ...
  const handleHotkeyRecord = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (keys.length > 0) {
      ...
      handleSettingChange('hotkeyCombo', hotkey)
  ...
  return (
    <div className="w-full h-screen bg-gradient-to-br from-slate-900 to-slate-800 p-6 overflow-y-auto">
      <div className="max-w-2xl mx-auto">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-700 text-white border border-slate-600 rounded px-3 py-2 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400"
                  />
                  <button
                    onClick={handleTestWorkerUrl}
                    disabled={testing === 'worker'}
                    className="px-3 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:bg-gray-600 transition text-sm font-medium"
                  >
                    {testing === 'worker' ? 'Testing...' : 'Test'}
                  </button>
                </div>
                <p className="text-xs text-gray-400 mt-1">
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={testing === 'hotkey'}
                  className="px-3 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:bg-gray-600 transition text-sm font-medium"
                >
                  {testing === 'hotkey' ? 'Testing...' : 'Test'}
                </button>
              </div>
        ...
        {/* Message Display */}
        {message.type && (
          <div className={`mt-4 border px-4 py-3 rounded-lg flex items-center gap-2 ${
        ...
        <div className="mt-8 p-4 bg-slate-900 rounded-lg border border-slate-700">
          <p className="text-xs text-gray-400">
            Maverick AI v0.1.0 • Settings synced with Electron Store • API keys secured in Worker proxy
          </p>
        </div>
      </div>
    </div>
